...
PURCHASE_TRANSPORT="${PURCHASE_TRANSPORT:-http}"
...
DHL_DIRECT_GAME_URL="${DHL_DIRECT_GAME_URL:-https://ol.dhlottery.co.kr/olotto/game/game645.do}"
...
if [[ -z "${ALLOWED_EMAIL}" ]]; then
  echo "ALLOWED_EMAIL is required"
  exit 1
fi
...
ENV_VARS="ALLOWED_EMAIL=${ALLOWED_EMAIL},ENABLE_REAL_PURCHASE=${ENABLE_REAL_PURCHASE},CONFIRM_PURCHASE=${CONFIRM_PURCHASE},PURCHASE_TRANSPORT=${PURCHASE_TRANSPORT},DHL_LOGIN_URL=${DHL_LOGIN_URL},DHL_GAME_URL=${DHL_GAME_URL},DHL_DIRECT_GAME_URL=${DHL_DIRECT_GAME_URL},BROWSER_HEADLESS=${BROWSER_HEADLESS},BROWSER_PLATFORM_SPOOF=${BROWSER_PLATFORM_SPOOF},BROWSER_NAVIGATION_TIMEOUT_MS=${BROWSER_NAVIGATION_TIMEOUT_MS},BROWSER_ACTION_DELAY_MS=${BROWSER_ACTION_DELAY_MS},PURCHASE_MAX_GAMES=${PURCHASE_MAX_GAMES},PUPPETEER_CACHE_DIR=${PUPPETEER_CACHE_DIR},CAPTURE_SCREENSHOT=${CAPTURE_SCREENSHOT}"
