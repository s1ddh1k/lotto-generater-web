...
const purchaseTransport = process.env.PURCHASE_TRANSPORT || 'http'
...
async function readPurchaseLayer(frame) {
  return frame.evaluate(() => {
    ...
    const receiptLayer = document.querySelector('#popReceipt')
    if (visible(receiptLayer)) {
      const message = receiptLayer?.textContent?.replace(/\s+/g, ' ').trim() || '구매 영수증이 발급되었습니다.'
      return { message, source: 'popReceipt' }
    }
...
async function readReportRows(frame) {
  return frame.evaluate(() => {
    const receiptRows = Array.from(document.querySelectorAll('#reportRow li'))
      .map(item => {
    ...
    if (receiptRows.length > 0) return receiptRows

    // The current Lotto 6/45 page keeps prepared games in rows A-E. The old
    // implementation only read receipt rows, so every prepared game appeared
    // missing even though the UI had accepted it.
    return ['A', 'B', 'C', 'D', 'E']
      .map(letter => {
        const selection = document.querySelector(`#selectGbn${letter}`)?.textContent?.trim() || ''
        if (!selection || selection === '미지정') return []

        return Array.from({ length: 6 }, (_, index) => {
          const node = document.querySelector(`#num${index}${letter}`)
          const raw = node instanceof HTMLInputElement ? node.value : node?.textContent || ''
          const number = Number.parseInt(raw.trim(), 10)
          return Number.isInteger(number) ? String(number).padStart(2, '0') : ''
        }).filter(Boolean)
      })
      .filter(game => game.length === 6)
  })
...
async function loginToDhlottery(page, trace) {
  ...
  const loginProbeTimeout = Math.min(navigationTimeoutMs, 15_000)
  try {
    await page.goto(dhlLoginUrl, { waitUntil: 'domcontentloaded', timeout: loginProbeTimeout })
  } catch (error) {
    const message = error instanceof Error ? error.message : 'navigation failed'
    logStep(trace, `login:navigation-delayed ${message}`)
  }

  try {
    await page.waitForSelector('#inpUserId', { timeout: loginProbeTimeout })
  } catch {
    ...
    throw new Error(`Login form was not found: ${JSON.stringify(diagnostics)}`)
  }
...
async function findGameSurface(page, timeout) {
  const deadline = Date.now() + timeout

  while (Date.now() < deadline) {
    if (await page.$('#checkNumGroup')) return page

    const iframe = await page.$('#ifrm_tab')
    const frame = iframe ? await iframe.contentFrame() : null
    if (frame && (await frame.$('#checkNumGroup'))) return frame
...
async function openGameFrame(page, trace) {
  const candidates = [...new Set([dhlGameUrl, dhlDirectGameUrl])]
  const probeTimeout = Math.min(navigationTimeoutMs, 15_000)
  let frame = null

  for (const url of candidates) {
    logStep(trace, `game:start ${url}`)
    try {
      await page.goto(url, { waitUntil: 'domcontentloaded', timeout: probeTimeout })
    } catch (error) {
      const message = error instanceof Error ? error.message : 'navigation failed'
      logStep(trace, `game:navigation-failed ${url}: ${message}`)
      continue
    }
    frame = await findGameSurface(page, probeTimeout)
    if (frame) {
      logStep(trace, `game:surface-ready ${url}`)
      break
    }

    logStep(trace, `game:surface-missing ${url}`)
  }

  if (!frame) {
    throw new Error('Lotto 6/45 game controls were not found on the wrapper or direct game page')
  }
...
async function addOneGame(frame, numbers, index, trace) {
  ...
  if (afterRows.length !== beforeRows.length + 1) {
    throw new Error(
      `Game line ${index + 1} was not added (expected ${beforeRows.length + 1} prepared games, got ${afterRows.length})`
    )
  }
}
...
async function submitPurchase(frame, trace) {
  ...
  const receiptVisible = await frame.evaluate(() => {
    const receipt = document.querySelector('#popReceipt')
    if (!(receipt instanceof HTMLElement)) return false
  ...
  if (!receiptVisible) {
    return {
      ...
      note: '최종 확인은 실행됐지만 구매 영수증을 확인하지 못했습니다. 구매 내역을 확인해 주세요.'
    }
  }
  ...
  return {
    status: 'purchase-submitted',
    note: '구매 영수증이 확인되었습니다.',
    blockerSource: 'popReceipt'
  }
}
...
async function runRealPurchase(payload) {
  ...
  if (purchaseTransport === 'http') {
    return runHttpPurchase(payload, {
      userId: dhlUserId,
      password: dhlUserPassword,
      loginUrl: dhlLoginUrl,
      directGameUrl: dhlDirectGameUrl,
