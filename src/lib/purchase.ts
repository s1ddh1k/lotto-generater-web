  ...
  switch (statusCode) {
    ...
    case 'verification-needed':
      return {
        ...
        title: '구매 내역 확인 필요',
        statusLabel: '확인 필요',
        fallback: '구매 확인은 실행됐지만 영수증을 확인하지 못했습니다. 구매 내역을 확인해 주세요.'
      }
    ...
    case 'limit-exceeded':
      return {
        ...
        title: '구매 한도 초과',
        statusLabel: '한도 초과',
        fallback: '온라인복권 주간 구매 한도를 초과했습니다.'
      }
    ...
    default:
      return {
        ...
        title: '구매 결과 확인 필요',
        statusLabel: '확인 필요',
        fallback: '구매 결과를 확정하지 못했습니다. 구매 내역을 확인해 주세요.'
      }
  }
}
...
export function derivePurchaseRecord(response: PurchaseApiResponse, options: PurchaseRecordOptions = {}): PurchaseRecord {
  ...
  const reportedStatus = firstNonEmpty(result?.status, response.status)
  const statusCode = error || response.ok === false || worker?.ok === false
    ? 'failed'
    : reportedStatus || 'verification-needed'
