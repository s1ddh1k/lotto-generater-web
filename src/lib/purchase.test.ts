...
describe('derivePurchaseRecord', () => {
  ...
  it('does not report success when the worker cannot verify a receipt', () => {
    const record = derivePurchaseRecord({
      ok: true,
      worker: {
        ok: true,
    ...
    expect(record.tone).toBe('warning')
    expect(record.statusLabel).toBe('확인 필요')
    expect(record.link).toBe('')
  })

  it('reports a worker error as failed even when no status code is returned', () => {
    const record = derivePurchaseRecord({
      ok: true,
      worker: {
        ok: false,
        error: '구매 요청 시간이 초과되었습니다.'
      }
    })

    expect(record.tone).toBe('error')
    expect(record.statusCode).toBe('failed')
    expect(record.statusLabel).toBe('실패')
    expect(record.title).toBe('구매 실패')
  })

  it('requires verification when a response has neither a status nor an error', () => {
    const record = derivePurchaseRecord({ ok: true })

    expect(record.tone).toBe('warning')
    expect(record.statusCode).toBe('verification-needed')
    expect(record.statusLabel).toBe('확인 필요')
  })

  it('does not claim an unknown status was processed', () => {
    const record = derivePurchaseRecord({
      ok: true,
    ...
    expect(record.tone).toBe('warning')
    expect(record.statusCode).toBe('new-backend-status')
    expect(record.statusLabel).toBe('확인 필요')
